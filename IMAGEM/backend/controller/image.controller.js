...
const putdata = async (req, res) => {
  try {
    const existingdata = await imgmodel.findById(req.params.id);
    if (!existingdata) {
      if (req.file.filename) {
        ...
        fs.unlink(filepath, (err) => {
          if (err) {
            console.log("failed to data", err);
          }
        });
      }
    ...
    if (req.file) {
      if (existingdata.image) {
        const filepath = path.join("upload", existingdata.image);
        console.log(filepath);
        fs.unlink(filepath, (err) => {
          if (err) {
            console.log("failed to data", err);
    ...
    const data = await imgmodel.findByIdAndUpdate(req.params.id, req.body, {
...
const deletedata = async (req, res) => {
  try {
    ...
    if (data.image) {
      const filepath = path.join("upload", data.image);
      console.log(filepath);
      fs.unlink(filepath, (err) => {
        if (err) {
          console.log("failed to data", err);
